...
export default function PlayerNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 flex border-t border-gray-800 bg-black/95 backdrop-blur-sm">
      <Link
        href="/"
        className={`flex-1 flex flex-col items-center justify-center py-3 gap-1 transition-colors ${path === '/' ? 'text-crimson' : 'text-gray-600 hover:text-gray-400'}`}
      >
      ...
      >
        ...
        <span className="text-[10px] font-bold uppercase tracking-wide">Design</span>
      </Link>
      <Link
        href="/vote"
      ...
      >
        ...
        <span className="text-[10px] font-bold uppercase tracking-wide">Vote</span>
      </Link>
    </div>
  );
}
